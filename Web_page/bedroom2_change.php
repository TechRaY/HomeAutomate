<?php
	...
	$result_ligth = mysqli_query($con,"select state from devices where appliances='light_bedroom_2'") or die(mysqli_error($con));
	$result_fan = mysqli_query($con,"select state from devices where appliances='fan_bedroom_2'") or die(mysqli_error($con));
	...
	{
		mysqli_query($con,"update devices set state='true' where appliances='light_bedroom_2'");
		fwrite($port,"H");
	}
	if($sent_url=="L"&&$row_light[0]=="true")
	{
		mysqli_query($con,"update devices set state='false' where appliances='light_bedroom_2'");
		fwrite($port,"h");
	}
	if($sent_url=="F"&&$row_fan[0]=="false")
	{
		mysqli_query($con,"update devices set state='true' where appliances='fan_bedroom_2'");
		fwrite($port,"E");
	}
	if($sent_url=="F"&&$row_fan[0]=="true")
	{
		mysqli_query($con,"update devices set state='false' where appliances='fan_bedroom_2'");
		fwrite($port,"e");
	}
	header("Location:bedroom2.php");
?>