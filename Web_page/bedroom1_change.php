<?php
   ...
   //echo $sent_url;
	...
	$result_ligth = mysqli_query($con,"select state from devices where appliances='light_bedroom_1'") or die(mysqli_error($con));
	$result_fan = mysqli_query($con,"select state from devices where appliances='fan_bedroom_1'") or die(mysqli_error($con));
	...
	{
		mysqli_query($con,"update devices set state='true' where appliances='light_bedroom_1'");
		fwrite($port,"I");
	}
	if($sent_url=="L"&&$row_light[0]=="true")
	{
		mysqli_query($con,"update devices set state='false' where appliances='light_bedroom_1'");
		fwrite($port,"i");
	}
	if($sent_url=="F"&&$row_fan[0]=="false")
	{
		mysqli_query($con,"update devices set state='true' where appliances='fan_bedroom_1'");
		fwrite($port,"E");
	}
	if($sent_url=="F"&&$row_fan[0]=="true")
	{
		mysqli_query($con,"update devices set state='false' where appliances='fan_bedroom_1'");
		fwrite($port,"e");
	}
	
	header("Location:bedroom1.php");
