...
/*		$con=mysql_connect("localhost","root","") or die ("Couldn't connect to database" . mysql_error());
		...
		$result_ligth = mysqli_query($con,"select state from devices where appliances='light_bedroom_2'") or die(mysql_error());
		$result_fan = mysqli_query($con,"select state from devices where appliances='fan_bedroom_2'") or die(mysql_error());
	...
	<body>
		<p class="light">
			...
			{
				$url="L";
				echo "<form action='bedroom2_change.php'  method='post'>";
