<html>
	...
	<?php
		...
		$result_ligth = mysqli_query($con,"select state from devices where appliances='light_kitchen'") or die(mysql_error());
		$result_fan = mysqli_query($con,"select state from devices where appliances='temperature_sensor'") or die(mysql_error());
	...
	<body>
		<p class="light">
			...
			{
				$url="L";
				echo "<form action='kitchen_change.php'  method='post'>";
			...
			?>
			<p class="fan">Temperature<br> Sensor</p>
			<?php
			if($row_fan[0]=='false')
			{
				$url="F";
				...
				echo "<input class='imgClassF' style='background-image: url(temp.png);' type = 'submit' value=''  />";
				echo "</form>";				
			}
			if($row_fan[0]=='true')
			{
